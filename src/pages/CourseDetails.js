...
export const CourseDetails = ({ course, index, randomizeOptions, setRandomizeOptions,
    ...
    const [invalidConstraintError, setInvalidConstraintError] = useState(false);
        ...
        if (gname.length === 0) {
            ...
            if (!newData[index].includes(gname)) {
                if (newData[index].length === course.groups.length - 1) {
                    ...
                    setInvalidConstraintError(true)
                } else {
    ...
    return (
        <>
        {duplicateCourse && (
            <ToastTemplate toastContext={'Course setup'} toastMessage={'You already exclude this course'} show={duplicateCourse} setShow={setDuplicateCourse} success={false}></ToastTemplate>
            )}
        {invalidConstraintError && (
            <ToastTemplate toastContext={'Course setup'} toastMessage={'You can\'t exclude all groups in this course'} show={invalidConstraintError} setShow={setInvalidConstraintError} success={false}></ToastTemplate>
        )}
                        ...
                        <div className="col-md-6">
                            ...
                            <ul className="list-group">
                                ...
                                {avoidGroupList[index].map((item) => (
                                    <li className="list-group-item list-group-item-danger d-flex justify-content-between align-items-center text-white" key={item}>
                                        {item}
                                        <button
